    ...
    useEffect(() => {
        const startTestSession = async () => {
            try {
                ...
                const response = await fetch(`${API_BASE_URL}/interviews/test-me?plan=FREE`);
    ...
    return (
        <div className="min-h-screen bg-[#0A0A0B] flex flex-col items-center justify-center p-8 text-center">
            ...
            <h2 className="text-2xl font-black text-white mb-2 uppercase tracking-widest animate-pulse">
                Launching FREE Assessment
            </h2>
            <p className="text-gray-500 font-bold uppercase tracking-tighter text-[10px]">
                Pre-generating 12 standardized questions • Bypassing authentication
            </p>
        </div>
    );
}
