...
export const JobCard: React.FC<JobCardProps> = ({ job, onViewDetails }) => (
  <Card className="group hover:border-blue-500 transition-all duration-300 border-2 border-transparent bg-white shadow-xl rounded-[2rem]">
    <div className="flex justify-between items-start mb-6">
      <div>
        <h3 className="text-lg font-black text-gray-900 group-hover:text-blue-600 transition-colors">{job.title}</h3>
        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">{job.role}</p>
      </div>
    ...
    <div className="pt-6 border-t border-gray-50 flex justify-between items-center">
      ...
      <Button 
        ...
        className="text-blue-600 text-xs font-black uppercase tracking-widest p-0 hover:bg-transparent"
      >
        View Detail &rarr;
      </Button>
    </div>
